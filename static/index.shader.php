...
<script id="vertexShaderGlo" type="x-shader/x-vertex">
...
void main() {
        ...
        viewVec[0] = viewVec[0] * -1.0;
        viewVec[1] = viewVec[1] * -1.0;
        viewVec[2] = viewVec[2] * -1.0;
...
<script id="fragmentShaderGlo" type="x-shader/x-vertex"> 
...
<script type="x-shader/x-fragment" id="fragmentShaderTexture">
	...
	void main() {
		gl_FragColor = texture2D(tDiffuse, vUv); // Displays Nothing
	}
</script>
...
function init() {
	...
	*/
	...
	var floorTexture = new THREE.ImageUtils.loadTexture( 'images/checkerboard.bmp' );
	...
	app.materials['glo'] = new THREE.ShaderMaterial({
		...
		vertexShader:   document.getElementById( 'vertexShaderGlo'   ).textContent,
		fragmentShader: document.getElementById( 'fragmentShaderGlo' ).textContent,
	...
	/*
	app.materials['shadertex'] = new THREE.ShaderMaterial({
		uniforms:	{
			tDiffuse: { type: "t", value: THREE.ImageUtils.loadTexture( "images/checkerboard.png" ) }
		},
		//attributes:	app.shadert_attribs,
		//blending:     THREE.MultiplyBlending,
		//depthTest:    true,
	...
	*/
	...
	//meshx.position.set(0,5,0);
	scene.add( meshx );
	
	/*
	var meshy = new THREE.Mesh( 
		new THREE.IcosahedronGeometry( 2.0, 3 ), 
		app.materials.shadertex
	);
	meshy.position.set(0,8,0);
	scene.add( meshy );
	...
	app.moon.position.set( 0.0, 10.0, 0.0 );
	scene.add(app.moon);
	*/
	...
	app.moonx = new THREE.Mesh( 
		new THREE.IcosahedronGeometry( 8.2, 5 ), 
		app.materials.glo
	);
	app.moonx.position.set( 0.0, 10.0, 0.0 );
	scene.add(app.moonx);
	...
	for( var o1 = 0; o1 < 224; o1++ ) {
		...
		node.sphere = new THREE.Mesh(  new THREE.SphereGeometry( 0.5, 10, 10 ), app.materials.glo );
		...
		node.sphere.position.set( o.x, o.y, o.z );
	...
	//scene.add( new THREE.Line( line_geometry, app.materials.glow ) );
