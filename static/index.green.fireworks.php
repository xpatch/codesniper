...
function init() {
	...
	function engineStart() {
		...
		app.engine = new ParticleEngine();
		app.engine.setValues({
			positionStyle    : Type.CUBE,
			//positionBase     : new THREE.Vector3(  0, 10, 0 ),
			positionBase     : new THREE.Vector3(  0, app.engine.randomValue( 10,5 ), 0 ),
			positionSpread   : new THREE.Vector3( 1, 1, 1 ), //This is the emitter box or sphere
			/*
			...
			*/
			...
			velocitySpread   : new THREE.Vector3( 100, 100, 100 ), //how much + or - from the base velocity this is traveling
			//velocitySpread   : new THREE.Vector3( 100, 20, 100 ),
			//velocitySpread   : new THREE.Vector3( 190, 1, 190 ),
			*/
			...
			particleTexture : new THREE.ImageUtils.loadTexture( 'images/particle/spark.png' ),
			sizeBase     : 1.0,
			/*
			sizeTween    : new Tween( [ 0, 0.3 ], [2.1, 0.1] ),
			colorTween   : new Tween( [ 0.0, 0.09 ], [ new THREE.Vector3( 0.4, 1.0, 1.0 ), new THREE.Vector3( 0.4, 1.0, 0.5 ) ] ),
			*/
			sizeTween    : new Tween( [ 0, 0.3 ], [0.3, 2.3] ),
			colorBase    : new THREE.Vector3( 0.4266, 1.0, 0.5 ), 
			colorSpread  : new THREE.Vector3( 0.10, 0.1, 0.0 ), 
			...
			colorTween   : new Tween([ 
				...
				new THREE.Vector3( 0.6266, 1.0, 0.5 ), 
				new THREE.Vector3( 0.6266, 1.0, 0.5 )
				//new THREE.Vector3( 0.6666, 1.0, 0.5 ), 
				//new THREE.Vector3( 0.5555, 1.0, 0.5 ) 

				/// Rainbow 
				//new THREE.Vector3( 0.0, 1.0, 0.5 ), 
				//new THREE.Vector3( 1.00, 1.0, 0.5 ) 

				//new THREE.Vector3( 0.47, 1.0, 0.5 ), 
				//new THREE.Vector3( 0.47, 1.0, 1.0 ), 
			...
			*/
			...
			//colorBase: new THREE.Vector3( 0.38333, 1, 0.9 ),

			blendStyle   	   : THREE.AdditiveBlending,
			particlesPerSecond : 29000,
			particleDeathAge   : 0.4,
			emitterDeathAge    : 0.01
			/* Sparkler */
			/*
			particlesPerSecond : 1900,
			particleDeathAge   : 0.4,
			emitterDeathAge    : 11000.01
			*/
		});
	...
	function boomStart( ) {
		var newBoom =  new ParticleEngine();
		newBoom.setValues({

			positionStyle    : Type.CUBE,
			positionBase     : new THREE.Vector3(  
				newBoom.randomValue( 0.0, 5.0 ), 
				newBoom.randomValue( 10.0, 3.0 ), 
				newBoom.randomValue( 0.0, 5.0 )
			),
			positionSpread   : new THREE.Vector3( 1.0, 1.0, 1.0 ), //This is the emitter box or sphere
			...
			accelerationBase : new THREE.Vector3( 0.0, 0.0, 0.0 ),
			particleTexture  : new THREE.ImageUtils.loadTexture( 'images/particle/spark.png' ),
			blendStyle   	 : THREE.AdditiveBlending,
	...
	app.crap = setInterval( boomStart, 400 );
	//app.crap = setInterval( engineStart, 1000 );
	...
	app.materials['floorz'] = new THREE.MeshLambertMaterial({ 
		color: 0xFFFFFF,
		//color: 0x404040,
		map: PATCH.images['checker']
	});
	...
	*/
	...
	if( app.floor == true ) {
		...
		var floor = new THREE.Mesh( floorgeo, app.materials.floorz );
...
function update() {
	...
	for( var p in app.particles ) {
		if( app.particles[p] ) {
			//if( app.particles[p].emitterAlive == false ) app.particles[p].destroy();
			if( app.particles[p].emitterAge > 1 ) app.particles[p].destroy();
			app.particles[p].update( dt * 0.5 );      
		}

	}
        if( app.engine ) app.engine.update( dt * 0.5 );      
